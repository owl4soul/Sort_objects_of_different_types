...
public class ObjectComparableWrapper<T> implements Comparable<ObjectComparableWrapper> {
	 ...
	 *
	 * ВНИМАНИЕ! Метод выдает равенство между двумя разными объектами, не являющимеся подклассами String или Integer,
	 * что вступает в противоречие с методом equals - это будет отражаться на коллекциях, использующих метод сравнения
	 * compareTo (например, TreeSet).
	 *
